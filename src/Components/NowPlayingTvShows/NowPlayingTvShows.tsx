...
export default function NowPlayingTvShows({ props }: any) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <>
          <div id="nowPlayingTvShows" className="container mb-3 d-flex flex-column align-items-center justify-content-center">
            <h2 className="mb-0 h3 align-self-start">Now Playing Tv Shows</h2>
          ...
          {nums.map((num: number, index: number): any => {
            return (
              <li key={index} className="page-item">
                <button className="page-link shadow-none text-dark-blue" onClick={() => getItems("tv", "on_the_air", setTvShows, num)}>
                  {num}
                </button>
              </li>
            );
